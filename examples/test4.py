...
def tf_octa_c(
    ...
    # Note: Rotated 180 means:
    # Upper local (y>0) becomes Lower global (y<0).
    # But for simplicity, the 'ip' is usually the Positive signal.
    # In Skill: 'ip' is at +y (relative to portxl?). 
    # Skill: `list(portxl float(fs)/2.0)` -> This is Positive Y.
    # My winding generator puts 'y_upper' at +Y. Rotated 180 -> -Y.
    # So if we want 'ip' at +Y, it must connect to the 'y_lower' (local) leg of the rotated octagon.
    # Let's align strictly with Skill coordinates:
    # ip: (portxl, fs/2).
    # in: (portxl, -fs/2 - wi).
