...
class SimulationWorkerThread(QThread):
	...
	def __init__(self, geometry_class, geometry_params, num_samples, cpu_cores, epochs, palace_executable):
		super().__init__()
...
class SimulationTrainingTab(QWidget):
	...
	def _build_ui(self):
		...
		self.step_label.setStyleSheet("font-size: 14px; font-weight: bold; color: #EBF4DD; padding: 5px;")
	...
	def run_orca_simulation(self):
		...
		self.step_label.setStyleSheet("font-size: 14px; font-weight: bold; color: #5A7863; padding: 5px;")
		self.reset_step_progress()
		if self.step_order:
	...
	def on_progress(self, step, current, total, message):
		...
		if "failed" in message.lower() or "error" in message.lower():
			color = "#FF5252"
		self.step_label.setStyleSheet(f"font-size: 14px; font-weight: bold; color: {color}; padding: 5px;")
	...
	def on_finished(self, success, message):
		...
		if success:
			...
			self.step_label.setStyleSheet("font-size: 14px; font-weight: bold; color: #90AB8B; padding: 5px;")
			QMessageBox.information(self, "Success", message)
		else:
			self.progress_bar.setVisible(False)
			self.step_label.setText("❌ Simulation Failed")
			self.step_label.setStyleSheet("font-size: 14px; font-weight: bold; color: #FF5252; padding: 5px;")
