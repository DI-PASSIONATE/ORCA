...
class SimulationTrainingTab(QWidget):
    ...
    def _build_ui(self):
        """Build the simulation and training tab UI."""
        layout = QVBoxLayout()
        ...
        self.geometry_display = QLabel(
            self.current_geometry_name or "Select geometry in main window"
        )
        ...
        sim_config_layout.addRow("Training Epochs:", self.epochs_spin)

        self.palace_exec_edit = QLineEdit()
        self.palace_exec_edit.setText(
            "apptainer exec ~/Documents/git/palace/palace.sif palace"
        )
        ...
        self.step_label.setStyleSheet(
            "font-size: 14px; font-weight: bold; color: #EBF4DD; padding: 5px;"
        )
    ...
    def on_geometry_changed(self, geometry_name):
        ...
        browse_stackup_btn.clicked.connect(
            lambda: self.browse_file(self.stackup_xml_edit, "XML Files (*.xml)")
        )
        ...
        default_simconfig = geometry_info.get("default_params", {}).get(
            "simconfig_filename"
        )
        ...
        browse_simconfig_btn.clicked.connect(
            lambda: self.browse_file(self.simconfig_edit, "SimConfig Files (*.simcfg)")
        )
    ...
    def reset_step_progress(self):
        ...
        self.step_state = {
            key: {"state": "todo", "current": 0, "total": 0}
    ...
    def update_step_widget(
        self, key: str, label: str, state: str, current: int, total: int
    ):
        ...
        icon_label.setStyleSheet(
            f"color: {color}; font-weight: bold; padding-right: 4px;"
        )
    ...
    def run_orca_simulation(self):
        ...
        self.step_label.setStyleSheet(
            "font-size: 14px; font-weight: bold; color: #5A7863; padding: 5px;"
        )
        self.reset_step_progress()
        if self.step_order:
        ...
        self.worker = SimulationWorkerThread(
            geometry_info["class"],
            geometry_params,
            num_samples,
            cpu_cores,
    ...
    def on_finished(self, success, message):
        ...
        if success:
            ...
            self.step_label.setStyleSheet(
                "font-size: 14px; font-weight: bold; color: #90AB8B; padding: 5px;"
            )
            ...
            self.step_label.setStyleSheet(
                "font-size: 14px; font-weight: bold; color: #FF5252; padding: 5px;"
            )
