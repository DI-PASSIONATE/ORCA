...
def _refresh_checkpoints(self):
    ...
    try:
        ...
        # Create engine to list checkpoints
...
def _on_checkpoint_selected(self):
    ...
    try:
        ...
        # info_text += f"Inputs ({len(inputs)}):\n"
        # for input_name, spec in inputs.items():
        #     info_text += f"  • {input_name}: {spec['shape']}\n"
        
        outputs = self.inference_engine.get_output_specs()
        # info_text += f"\nOutputs ({len(outputs)}):\n"
        # for output_name, spec in outputs.items():
        #     info_text += f"  • {output_name}: {spec['shape']}\n"
        
        self.model_info_label.setText(info_text)
        
        # Populate input parameter fields
...
def _populate_input_fields(self, input_specs: dict):
    ...
    # Every input is a single float; render compact scalar fields
...
def _run_inference(self):
    ...
    try:
        ...
        self.infer_button.setEnabled(False)
        self.inference_progress.setRange(0, 0)  # Indeterminate progress
...
def create_inference_tab(self):
    """Create the inference and prediction tab with full ONNX support."""
    tab = QWidget()
    layout = QVBoxLayout()
    ...
    self.checkpoint_combo.currentTextChanged.connect(lambda: self._on_checkpoint_selected())
    refresh_btn = QPushButton("Refresh")
    refresh_btn.clicked.connect(lambda: self._refresh_checkpoints())
    ...
    self.infer_button.clicked.connect(lambda: self._run_inference())
    self.infer_button.setEnabled(False)
    ...
    # Bind helper methods to the instance
    self._refresh_checkpoints = _refresh_checkpoints.__get__(self, type(self))
    self._on_checkpoint_selected = _on_checkpoint_selected.__get__(self, type(self))
    self._populate_input_fields = _populate_input_fields.__get__(self, type(self))
    self._run_inference = _run_inference.__get__(self, type(self))
    self._on_inference_finished = _on_inference_finished.__get__(self, type(self))
