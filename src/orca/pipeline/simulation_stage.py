...
class PalaceSimulator(PipelineStage):
    ...
    def run(self, context: Dict[str, Any], progress_callback: Optional[Callable[[float, str], None]] = None) -> Dict[str, Any]:
        ...
        result_data.drop(columns=["data_dir", "sim_path", "config_name"], inplace=True, errors='ignore')
        ...
        for i, (index, row) in tqdm.tqdm(enumerate(palace_data.iterrows()), total=len(palace_data), desc="Palace Simulations"):
            ...
            if not success:
                ...
                logger.error(f"Simulation for config {palace_config_name} failed. Removing from results.")
