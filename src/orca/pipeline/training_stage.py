...
class ModelTrainer(PipelineStage):
    ...
    def run(self, context: Dict[str, Any], progress_callback: Optional[Callable[[float, str], None]] = None) -> Dict[str, Any]:
        ...
        logger.info(f"Loaded {len(train_dataset)} training samples and {len(val_dataset)} validation samples for model training. Beginning training...")

        trained_model = train_model(
            train_dataset=train_dataset, 
            val_dataset=val_dataset, 
            model=geometry.model, epochs=15, 
            batch_size=32, 
            learning_rate=1e-3, 
